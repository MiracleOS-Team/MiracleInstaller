...
function disk_configuration() {{
	create_classic_single_disk_layout swap={swap} type={boot.type} luks={disk.encryption.enabled} root_fs={disk.rootfs} {disk.device}
...
LOCALES={system.locales.enabled}
...
SYSTEMD_NETWORKD={starter.systemd.networkd.enabled}
...
SYSTEMD_NETWORKD_DHCP={starter.systemd.networkd.dhcp.enabled}
...
SYSTEMD_INITRAMFS_SSHD={initramfs.sshd.enabled}
...
GENTOO_MIRROR="https://gentoo.mirrors.ovh.net/gentoo-distfiles/"
...
STAGE3_VARIANT={starter.stage3.variant}
...
SELECT_MIRRORS_LARGE_FILE={mirrors.select.largefiles}
